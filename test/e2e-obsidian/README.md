...
npm run test:e2e:obsidian:tag-tree-style
```

The three test commands invoke the preparation command automatically. Running it separately is useful for checking the selected version and executable paths. Importing the test-session package never downloads Obsidian; only the explicit preparation command performs network and filesystem writes.
